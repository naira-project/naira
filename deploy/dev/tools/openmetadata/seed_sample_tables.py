...
Mirrors the MLflow dummy-model seeder: it gives the local OpenMetadata instance
some real metadata so the Naira catalog plugin has tables to ingest from
/api/v1/tables.

On top of the raw source tables it seeds two derived tables and wires
table- and column-level lineage edges between them, so the OpenMetadata
lineage view (and any Naira lineage feature) has a real graph to render
instead of disconnected nodes.

OpenMetadata runs with the `basic` auth provider in the dev cluster, so all API
calls require a JWT. The script logs in as the default admin to obtain one, then
creates the service/database/schema/tables and lineage with idempotent PUTs
(createOrUpdate), so running it repeatedly is safe.
"""
